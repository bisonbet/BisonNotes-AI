...
- **A marker is retired once its delete has had `deletionMarkerRetentionInterval` to reach every device.** Nothing used to remove an applied marker, so every delete the user had ever made was replayed on every sync and the preflight grew without bound. Past the window, a marker whose target is already gone locally is deleted; a marker with no usable `deletedAt` is never aged out.
- **An edit that lands more than `deletionReviveGraceInterval` after a delete beats that delete.** The tombstone is withdrawn and the item uploads again on the same pass. The grace window absorbs cross-device clock skew; a marker with no usable `deletedAt` never revives anything. The `.importedAudio` marker is the sole exception and is never withdrawn: it tombstones the audio link rather than a row, so the recording's own `lastModified` is the only timestamp available, and a rename — or the restore leg stamping the cloud's value back down — would read as "the audio came back". Re-importing audio creates a new recording row with its own id, so nothing legitimate is stranded.
- **A deleted imported placeholder must reach the other devices, not just CloudKit.** Removing an imported item's temporary audio raises an `.importedAudio` deletion marker alongside the cloud field clear; applying it locally unlinks `recordingURL` and removes the placeholder file while keeping the recording and its summary. Clearing only the cloud fields was a one-way instruction that no other device could act on — and because an absent `audioSignature` is exactly what puts a recording into `recordingsNeedingAudioOnly`, the clear itself invited those devices to re-upload the asset, which the restore leg then reinstalled on the device that had deleted it. Relatedly, the restore leg must be able to *clear* `recordingURL`, not only write it: when the cloud copy wins and carries neither a `recordingURL` nor any audio field, the local row is unlinked too.
...
- **Deletions are planned in memory, then issued once.** `applyiCloudDeletionMarkers` reads the manifest and the legacy `CD_EnhancedSummary` records once for the whole marker set (`makeDeletionWorkspace`), accumulates a `CloudDeletionPlan`, and commits it as one batched delete, one manifest delta and one save. Doing that work per tombstone is what made a routine pass fetch 10,246 records for 155 live ones and spend 370 of its 378 seconds in `applyInboundTombstones`. Nothing in the planners may touch the network.
- **A replayed tombstone must cost almost nothing.** A marker replays on every sync until `deletionMarkerRetentionInterval` closes, so the replay — not the first application — is the common case, and three things keep it cheap. `makeDeletionWorkspace` fetches with `deletionWorkspaceKeys`, because the planners read only `recordingId`, `transcriptId` and `summaryId`; fetching whole records pulled every transcript body, every summary body and every recording's `audioAsset` down on every run to read one string from each. `contentRecordIDsWorthDeleting` drops manifest-managed names the manifest no longer holds, since those are provably gone already — legacy `CD_EnhancedSummary` ids and an untrusted manifest are exempt, and `markerIDsToWithdraw` is kept apart from `recordIDsToDelete` so a withdrawal, which the manifest never names, is never filtered away. And `legacySummarySyncRecords` records an empty result in `legacySummaryRecordsAbsentKey`, because otherwise `needsLegacySummaryRecords` put a full type scan on the routine path forever; only a scan that completed may claim an absence, and writing a legacy record or erasing the account clears it.
- **`deleted` in the metrics line is not a count of removals.** CloudKit reports a delete of a missing record as success, and the executor folds it into `deleted` because already-absent is the state the caller wanted, so `recordsAlreadyAbsent` is reported separately as `alreadyGone=`. Without it a steady `deleted=62` could equally mean two devices trading uploads and deletions forever or sixty-two no-op replays — opposite problems, one log line.
