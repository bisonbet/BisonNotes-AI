...
- **A replayed tombstone must cost almost nothing.** A marker replays on every sync until `deletionMarkerRetentionInterval` closes, so the replay — not the first application — is the common case, and several safeguards keep it cheap. `makeDeletionWorkspace` fetches with `deletionWorkspaceKeys`, because the planners read only `recordingId`, `transcriptId` and `summaryId`; fetching whole records pulled every transcript body, every summary body and every recording's `audioAsset` down on every run to read one string from each. Records that a planner saves are refetched in full first. `contentRecordIDsWorthDeleting` filters only manifest-derived replay names; explicit ids from a deletion request are always issued because content and manifest writes are separate, while legacy `CD_EnhancedSummary` ids and an untrusted manifest are exempt. `markerIDsToWithdraw` remains separate so a withdrawal is never filtered away. Finally, `legacySummarySyncRecords` uses `recordsOrThrow()` and keeps only a short-lived absence memo; a failed or expired scan is retried, and writing a legacy record or erasing the account clears it.
