...
- In-app Help is version-aware. `BisonNotesDocumentation` derives both the release-guide slug and the section anchor from the installed marketing version (3.0 → `/bisonnotes-ai-v3-0/#bn30-ai`), and falls back to the unversioned landing page with no fragment when the version cannot be read.
- v2.4 dead code was removed across the app and test targets. The implementation plan is retained in Git history; current documentation is indexed in [docs/README.md](docs/README.md).
