...
final class LibraryRepositoryContractTests: XCTestCase {
    ...
    func testCoreDataRepositoryReturnsStorageNeutralRecordingSnapshot() async throws {
        ...
        XCTAssertEqual(recordings[0].locationAccuracy, 7.0)
        XCTAssertEqual(recordings[0].locationAddress, "Fixture address")
        XCTAssertEqual(recordings[0].locationLatitude, 39.25)
        XCTAssertEqual(recordings[0].locationLongitude, -76.71)
        XCTAssertEqual(
            recordings[0].locationTimestamp,
    ...
    func testCoreDataRepositoryUpdatesRecordingDateAndLocationWithRevisionGuards() async throws {
        let directory = try TestHelpers.createTemporaryDirectory()
        let fixture = try SQLiteMigrationCoreDataSourceFixtureFactory.make(
            at: directory.appendingPathComponent("repository-recording-edit.sqlite"),
            version: .active
        )
        defer {
            try? SQLiteMigrationCoreDataSourceFixtureFactory.close(
        ...
        XCTAssertEqual(dated.recordingDate, Date(timeIntervalSinceReferenceDate: 300))

        let location = LibraryRecordingLocationSnapshot(
            latitude: 40,
            longitude: -75,
            timestamp: Date(timeIntervalSinceReferenceDate: 302),
            accuracy: 5,
            address: "Contract address"
        )
        let located = try await repository.updateRecordingLocation(
            LibraryRecordingLocationUpdateCommand(
                reference: LibraryRecordingReference(storageID: dated.storageID),
                location: location,
        ...
        do {
            _ = try await repository.updateRecordingLocation(
                LibraryRecordingLocationUpdateCommand(
                    reference: LibraryRecordingReference(storageID: dated.storageID),
                    location: location,
                    expectedLastModified: dated.lastModified,
            ...
            XCTFail("Expected stale location update to fail")
        } catch let error as LibraryRepositoryError {
            XCTAssertEqual(
                error,
                .staleRecording(
                    reference: dated.storageID,
                    expected: dated.lastModified,
                    actual: located.lastModified
                )
            )
        }
    }
    ...
    func testCoreDataRepositoryDeletesTranscriptOnlyAndQueuesCloudRemovalIntent() async throws {
        let directory = try TestHelpers.createTemporaryDirectory()
        let fixture = try SQLiteMigrationCoreDataSourceFixtureFactory.make(
            at: directory.appendingPathComponent("repository-transcript-delete.sqlite"),
            version: .active
        )
        defer {
            try? SQLiteMigrationCoreDataSourceFixtureFactory.close(
        ...
        let deleted = try await repository.deleteTranscript(
        ...
        let pendingMutations = try await repository.fetchPendingCloudMutationSnapshots()
        let transcriptRemovals = pendingMutations.filter {
            $0.kind == PendingCloudMutationKind.transcriptRemoval.rawValue
        }
        XCTAssertEqual(transcriptRemovals.count, 1)
        XCTAssertEqual(transcriptRemovals[0].targetID, transcriptID.uuidString.lowercased())
        XCTAssertEqual(transcriptRemovals[0].recordingLegacyID, recordingID.uuidString.lowercased())
        XCTAssertEqual(transcriptRemovals[0].requestedAt, requestedAt)

        let pendingMutationCountAfterDelete = try await repository.fetchPendingCloudMutationSnapshots().count
        let repeatedDelete = try await repository.deleteTranscript(
            LibraryTranscriptDeleteCommand(
                id: transcriptID,
                requestedAt: requestedAt
            )
