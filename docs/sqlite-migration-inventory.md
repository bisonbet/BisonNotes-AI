...
outbound preserve-summary file-manager order and final production media/file
journal remain separate; no repository backend is wired to startup.
...
failure. Archive-location reads, restore and clear-flags remain direct legacy
paths, while the full production media/file journal remains separate. The
standalone suite passes 101/101 and the macOS app-hosted build-for-testing
check passes; the current full iOS scheme build is blocked by the pre-existing
