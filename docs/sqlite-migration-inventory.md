...
failure. Archive-location reads, file-provider restore and source deletion
remain direct legacy paths, while the full production media/file journal
remains separate. The recording-file cleanup checkpoint `fde51735` makes
...
await the repository path; file-provider restore, archive-location reads and
source deletion remain pending the durable media/file-operation boundary. The
standalone suite passes 102/102 and the macOS app-hosted build-for-testing
