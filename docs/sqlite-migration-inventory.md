...
Current processing-job checkpoint: `880ad8ea`.
...
adapter. `BackgroundProcessingManager` routes asynchronous status transitions,
stale-job reconciliation, rerun/duplicate cleanup, manual terminal cleanup and
clear-all enumeration/deletion through processing-job repository commands.
Synchronous job creation, the batch terminal cleanup helper and startup crash
reconciliation remain on their existing paths until their lifecycle semantics
are separately defined. The AI file-renaming workflow remains separate until
its surrounding file operations have a journaled repository command. Neither
backend is wired to startup or user-data migration.
...
| Processing-job update/delete commands | `LibraryProcessingJobUpdateCommand` and `LibraryProcessingJobDeleteCommand` address a job by storage or legacy ID, validate finite dates and apply an optimistic `lastModified` guard. Updates distinguish preserving an optional error/completion value from intentionally clearing it. Core Data commits each command in one context save; SQLite commits each in one GRDB transaction and emits one `processingJob` change. `BackgroundProcessingManager` routes asynchronous status transitions, stale-job reconciliation, rerun/duplicate cleanup, manual terminal cleanup and clear-all enumeration/deletion through the Core Data adapter. | Two host tests cover update set/preserve/clear behavior, delete durability, stale/not-found rejection, exact revisions and no revision on rejection; the app-hosted contract covers both adapters against the active compiled model. Job creation, batch terminal cleanup and synchronous startup crash reconciliation remain direct until their lifecycle semantics are separately defined. |
...
maintenance gate by every source mutation caller, and add production final
root selection/source-retention scheduling and Watch/share caller integration
