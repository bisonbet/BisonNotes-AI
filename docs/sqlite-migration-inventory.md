...
progress, cancellation pause, definitive failure checkpoints and closed-snapshot
verification. Production source/settings acquisition and coordinator wiring,
...
standalone runtime harness has passed thirty-three disposable macOS tests; the
...
| Resumable metadata coordinator | `SQLiteMigrationCoordinator` validates a closed snapshot, finds the newest matching pending/running run after reopen, emits progress after committed batches, leaves cancellation resumable, records definitive conflicts as failed with a generic durable message, and verifies the destination before completion. | Three host tests cover progress, reopen/resume and conflict failure. It is not wired to production source acquisition, settings, media, startup or an active user generation. |
...
the startup contract, and expand command/error coverage before production
