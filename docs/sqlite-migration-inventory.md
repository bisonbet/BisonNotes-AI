...
constitute SQLite activation or OS-managed background scheduling. The next
inventory update must expand command/error coverage, finish final media-root
selection and generic transfer/receipt/source-retention scheduling, convert or
explicitly exclude every remaining direct source mutation caller from the
maintenance-gate boundary, and add Watch/share integration before production
services can depend on the complete repository.
