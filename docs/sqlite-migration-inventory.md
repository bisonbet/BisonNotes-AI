...
Current processing-job checkpoint: `6816adc8`.
...
`17abdac3` after its call sites were eliminated. Startup crash reconciliation remains on its existing path until
its lifecycle semantics are separately defined. The AI file-renaming workflow
remains separate until its surrounding file operations have a journaled
repository command. Neither backend is wired to startup or user-data migration.
...
| Processing-job create/update/delete commands | `LibraryProcessingJobCreateCommand`, `LibraryProcessingJobUpdateCommand` and `LibraryProcessingJobDeleteCommand` address a job by stable storage or legacy ID, validate command values and apply an optimistic `lastModified` guard where a row is being changed or removed. Creation preserves the initial status, progress, model, error, completion and start-time values, optionally resolves a stable recording reference and rejects duplicate UUIDs. Updates distinguish preserving an optional error/completion value from intentionally clearing it. Core Data commits each command in one context save; SQLite commits each in one GRDB transaction and emits one `processingJob` change. `BackgroundProcessingManager` routes synchronous creation plus asynchronous status transitions, stale-job reconciliation, rerun/duplicate cleanup, manual terminal cleanup and clear-all enumeration/deletion through the Core Data adapter. SQLite canonicalizes UUID legacy references for migrated rows. The terminal cleanup command matches legacy casing/whitespace and records one durable delete change per deleted row. | Four focused host tests cover create/link/duplicate rejection, terminal cleanup, update set/preserve/clear behavior, delete durability, stale/not-found rejection, exact revisions and no revision on rejection; the app-hosted contract covers the Core Data create/terminal-cleanup paths and both adapters' update/delete paths against the active compiled model. Synchronous startup crash reconciliation remains direct until its lifecycle semantics are separately defined. |
