...
next inventory update must expand command/error coverage, connect the durable
archive-restore journal to a selected production SQLite store and bookmark/root
lifetime, convert or explicitly exclude every remaining direct source mutation
caller from the maintenance-gate boundary, and add production final root
selection/source-retention scheduling and Watch/share caller integration before
