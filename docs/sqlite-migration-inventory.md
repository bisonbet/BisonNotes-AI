...
Current processing-job checkpoint: `a554fe50`.
...
and stale-job reconciliation through a processing-job update command; lifecycle-
aware creation/deletion and synchronous startup crash reconciliation remain on
their existing paths until those commands are defined. The AI file-renaming
workflow remains separate until its surrounding file operations have a
journaled repository command. Neither backend is wired to startup or user-data
migration.
...
| Processing-job update command | `LibraryProcessingJobUpdateCommand` addresses a job by storage or legacy ID, validates finite progress and dates, applies an optimistic `lastModified` guard, and distinguishes preserving an optional error/completion value from intentionally clearing it. Core Data commits the patch in one context save; SQLite commits it in one GRDB transaction and emits one `processingJob` change. `BackgroundProcessingManager` routes asynchronous status transitions and stale-job reconciliation through the Core Data adapter. | One host test covers set/preserve/clear behavior, stale rejection, exact revisions and no revision on rejection; the app-hosted contract covers the Core Data adapter against the active compiled model. Job creation/deletion and synchronous startup crash reconciliation remain direct until lifecycle-aware commands are defined. |
