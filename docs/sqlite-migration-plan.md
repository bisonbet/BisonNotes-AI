...
archive caller, Documents-relative destination publication, bounded startup
retry and iOS background-processing scheduling,
and a version-8 generic media-transfer metadata-acknowledgement state with a
bounded caller metadata descriptor, receipt/retention gating and crash-retry
coverage,
...
The current provider archive-restore lifecycle checkpoint is `9cddca7c`
(`feat: schedule archive restore retries`), following `be23b99f`. iOS now
registers the dedicated permitted `BGProcessingTask` identifier
`com.bisonai.archive-restore`; incomplete journal work requests a bounded
follow-up, and activation retries run on both iOS and native macOS. A background
task launch also binds the repository coordinator before retrying, so the path
does not depend on a SwiftUI scene having been created. Expiration cancels the
archive pass and reports an unsuccessful task. This is scheduling and source
evidence only: real-device task delivery, expiration timing, power behavior and
...
root-selection gap. The Watch caller now deliberately selects Documents as its
pre-cutover destination; generic Share media remains a separate caller. The
generic transfer journal also has a separate metadata-acknowledgement phase.

The current generic media metadata-acknowledgement checkpoint is `04e962b2`
(`fix: require metadata acknowledgement for media retention`), following
`ef52505d` (`feat: gate media receipts on metadata acknowledgement`). Schema v7 adds
`metadataState` and `metadataAcknowledgedAt` to generic file operations. New
transfers remain in `pending` until a caller-supplied, idempotent metadata
transaction succeeds; only then can the source-transfer receipt be committed
and source retention become eligible. Reopen recovery normalizes an interrupted
metadata claim, retries failed callbacks without copying a verified destination
again, and records a receipt without repeating the callback when metadata was
already acknowledged. Pre-v7 completed rows are marked `legacy` so the schema
upgrade does not invent acknowledgement evidence. The standalone suite passes
129/129 and the macOS app-hosted build-for-testing check passes. At that v7
checkpoint this remained an isolated API boundary: production Watch/share
metadata callbacks, final media root selection, scheduling and signed-device
validation were still open; no SQLite cutover or live user data was used.

The current production Watch media-intake checkpoint is `d80fa3bc` (`feat:
journal Watch media intake safely`). Schema v8 adds a capped, caller-owned
metadata descriptor to each generic file operation so an interrupted metadata
acknowledgement can be reconstructed after reopen without placing audio bytes
in the journal. Watch transfers now move synchronously into persistent
Application Support/WatchTransferStaging, use stable recording-derived journal
identities and Documents-relative destinations, copy and verify audio through
the generic media runtime, commit Core Data metadata with the Watch recording
UUID as an idempotent identity, record the receipt, and remove the staged
source only after a verified retention pass. A bounded retry runs during
handler setup and app activation; the source remains available when any step
fails. Share/Inbox import cleanup now removes only sources reported as
successfully committed and retains failed or unsupported inputs. The full
standalone suite passes 132/132 and the macOS app-hosted build-for-testing
check passes. The iOS scheme remains blocked before app/test compilation by
the pre-existing Watch Widget `accessoryCorner` availability error; no signed
device, live user store or live CloudKit account was used.
...
- Share/background caller gates and the remaining file-operation/media
  integration: final production root selection for post-cutover media, generic
  Share receipt/retention integration, source-retention scheduling beyond the
  Watch activation retry, and startup progress-screen wiring. Watch intake now
  uses the generic media transfer/receipt/retention boundary; Share/Inbox
  cleanup retains failed inputs but has not yet adopted that journal. Provider
  archive restore is journaled in production through the existing Documents
  path with bounded startup/activation retry and a dedicated iOS
  processing-task request. Signed-device scheduling validation, generic
  background-media scheduling, and final SQLite media-root selection remain
  open.
...
the now-journaled provider restore and generic transfer contract: qualify
signed-device background delivery, define the final app-owned media roots,
connect Share media to the same metadata/receipt/retention journal, and add
bounded background scheduling. Watch intake already uses that boundary with
activation retry. The provider restore retry pass is now bounded
...
durable production path and generic media-caller integration remains open. After
...
Share/background callers without allowing them into first-boot activation yet,
and close the remaining Phase 0/1 evidence and caller-gate gaps.
