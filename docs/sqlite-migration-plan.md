...
production transcription callers are implemented, but no SQLite migration is
enabled**.
...
build-for-testing checks pass, and the host suite passes 85 tests. No repository
...
72/72 at that checkpoint; the current full suite passes 83/83.
...
- The first write commands are now explicit: recording rename references support
  ...
  and updates both rows atomically. Core Data and SQLite adapters return
  ...
  persistence uses the async transcript command; host and app-hosted contract
...
- Production coordinator/source/settings acquisition wiring beyond the
  ...
  repository contracts (including summaries, recording creation/deletion and
...
| 3: Repository boundary | **Started:** immutable snapshots for all six metadata entities, typed allowlisted settings adapters, recording rename and cloud-sync commands, transcript upsert, processing-job create/update/delete, terminal-cleanup and crash-recovery commands, Core Data and SQLite adapters, durable SQLite/Core Data observation adapters, the read-only Core Data migration source reader, and disposable contract tests are in place. `AudioPlayerView`, `SummaryDetailView`, `EditableTranscriptView` and summary-regeneration paths now use the Core Data adapter for display-name-only writes; synchronous background job creation, asynchronous status/reconciliation, terminal cleanup and startup crash reconciliation use the processing-job adapter; production transcription persistence uses the transcript-upsert adapter. Startup subscription, file-owning commands, and the remaining `AppDataCoordinator`, `RecordingWorkflowManager`, imports/summary/archive services, UI, cloud store access, fixtures and previews remain. | Core Data backend passes unchanged behavior plus shared repository contract tests. Managed objects/contexts confined to adapters and the legacy importer; all callers/targets audited. |
