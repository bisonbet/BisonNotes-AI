...
durable per-row delete changes in SQLite. Startup crash reconciliation remains
direct Core Data until its lifecycle semantics are separately defined; the
AI/workflow file-owning rename path also remains separate, and no audio file
...
The current processing-job checkpoint is `6816adc8` (`feat: route terminal job
cleanup through repository`), followed by cleanup commit `17abdac3`
(`refactor: remove obsolete direct job cleanup helper`), and following
`0ce30d1e` (`feat: route processing
job creation through repository`), `880ad8ea` (`feat: route processing job
deletion through repository`) and `a554fe50` (`feat: route background job
updates through repository`). `LibraryProcessingJobCreateCommand`,
...
processing-job delete change per deleted SQLite row. Startup crash reconciliation
remains direct until its lifecycle semantics are separately defined. Neither
backend is selected for production startup.
...
72/72 at that checkpoint; the current full suite passes 82/82.
...
- Disposable contract coverage now exercises the SQLite adapter against the
  ...
  82 tests with zero failures, including a disposable v2-to-v4 upgrade
...
- Synchronous startup crash reconciliation for processing jobs still uses direct
  Core Data APIs. The create/update/delete and terminal-cleanup commands cover
  job creation, asynchronous status transitions, rerun/duplicate cleanup,
  terminal cleanup and clear-all deletion, but crash reconciliation still needs
  lifecycle-aware semantics before the maintenance gate can claim complete
  background-caller coverage.
...
| 3: Repository boundary | **Started:** immutable snapshots for all six metadata entities, typed allowlisted settings adapters, recording rename and cloud-sync commands, processing-job create/update/delete and terminal-cleanup commands, Core Data and SQLite adapters, durable SQLite/Core Data observation adapters, the read-only Core Data migration source reader, and disposable contract tests are in place. `AudioPlayerView`, `SummaryDetailView`, `EditableTranscriptView` and summary-regeneration paths now use the Core Data adapter for display-name-only writes; synchronous background job creation, asynchronous status/reconciliation and terminal cleanup paths use the processing-job adapter. Startup subscription, crash-reconciliation commands, file-owning commands, and the remaining `AppDataCoordinator`, `RecordingWorkflowManager`, imports/transcript/summary/archive services, UI, cloud store access, fixtures and previews remain. | Core Data backend passes unchanged behavior plus shared repository contract tests. Managed objects/contexts confined to adapters and the legacy importer; all callers/targets audited. |
...
current host-independent suite passes 82 tests with 0 failures; iOS `build-for-testing`
