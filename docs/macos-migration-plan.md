...
- **Phase 3 — DONE.** The app-wide presentation audit covers all 60 sheet/full-screen sites. Summaries, transcripts, players, library, location, and processing views use movable/resizable native windows where appropriate, while true modal tasks remain bounded sheets. The native app has a dedicated Settings window, standard File/Edit commands and shortcuts, a resizable main window, and a complete Mac icon set. Native macOS, iOS Simulator, and Mac Catalyst builds, the iOS unit-test target, SwiftLint, and a native Release archive are green. User testing confirmed whole-window summary/transcript scrolling, nested-summary dismissal, artifact-free microphone recording, and `StartRecordingIntent` from Shortcuts with the app both closed and open. Detailed evidence is in `docs/macos-phase-3-exit-report.md`.
- **Phase 4.1 — DONE** (commit `2bbbced7`). A populated Catalyst install was replaced by the native app without losing recordings, transcripts, summaries, documents, or settings. The native app reopened the same persistent store and sandbox data; no relocation path was required. Detailed evidence is in `docs/macos-phase-4.1-data-continuity-report.md`.
- **Phase 4.2 recording hardening — IMPLEMENTED, SIGNED HARDWARE RETEST PENDING.** The beta exposed a present-but-silent USB input: AVAudioEngine started, but no microphone buffers reached the scratch file, so the UI displayed a 12-minute recording that could not be finalized. Mac capture now waits for the first successful scratch-file write before reporting that recording started, detects initial silence, later stalls, and write failures, and automatically rebuilds the input engine. Finalization validates microphone and system tracks independently, saves either usable track, and preserves all source media in Application Support if export still fails. Diagnostic exports include a recovery inventory. Automated logic tests and the unsigned native build are green; the Poly Sync 10 and signed TestFlight long-recording cases must be repeated.
...
- **Next: continue Phase 4.2** — repeat the signed recording matrix (including the Poly Sync 10 failure case), then continue the Mac beta/TestFlight soak and outstanding runtime cases: microphone changes during recording, long hidden-window processing, archive bookmark restoration, and the full Settings destination walkthrough.
